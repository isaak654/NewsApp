    ...
    private static List<News> extractNewsFromJson(String jsonResponse) {
        ...
        try {
            ...
            if (root.has("response")) {
                // Create a JSONObject from the key called "response"
                JSONObject response = root.getJSONObject("response");
                if (response.has("results")) {
                    //Extract from the JSONArray with the key "results"
                    ...
                    for (int i = 0; i < newsResults.length(); i++) {
                        //Get a single article and position it within the list of news
                        JSONObject currentNews = newsResults.getJSONObject(i);
                        // Extract the value for the key called "webTitle"
                        String title = currentNews.getString("webTitle");
                        //Extracts the String associated with the key "sectionName"
                        ...
                        // Create a new {@link News} object with the title, category, date,
                        // and url from the JSON response.
                        News newsItem = new News(title, category, url, date);
                        newsItems.add(newsItem);
                    }
                }
            } else {
                Log.v(LOG_TAG, "Cannot find json Object");
            }
